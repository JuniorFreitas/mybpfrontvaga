  ...
  <span>
    <template v-if="!template">
      <button
        ...
        :class="
          classBlock
            ? 'btn btn-sm btn-block btn-outline-primary'
    ...
    <div class="table-responsive" v-show="lista.length" v-if="!simples">
      <table class="table table-bordered table-hover table-condensed">
        ...
        <draggable
          v-model="lista"
          element="tbody"
          :options="{ draggable: '.linha', handle: '.mover' }"
        >
          <tr
            class="linha"
            :class="{ 'bg-warning': arquivo.falhou }"
            :key="index"
          ...
          >
            <td class="text-center">
              ...
              <span
                v-show="
                  !arquivo.imagem &&
                  (!arquivo.chave || (arquivo.enviado && !arquivo.falhou))
                "
              >
                <svg
                  version="1.1"
            ...
            <td class="text-center">
              ...
              <button
                class="btn btn-sm btn-secondary mover"
                v-show="
                  (!arquivo.chave && ordenar) ||
                  (arquivo.enviado && !arquivo.falhou && ordenar)
                "
                v-if="!leitura"
              >
              ...
              <div
                class="progress"
                v-show="arquivo.enviando || arquivo.aguardando"
              >
                <div
                  ...
                  aria-valuemax="100"
                >
                  <span v-if="arquivo.enviando"
                    >{{ arquivo.pctProgresso }}%</span
                  >
                </div>
              </div>
  ...
  methods: {
    remover(index) {
      ...
      if (this.lista[index].id && this.lista[index].temporario == true) {
        ...
        Api.delete(this.lista[index].urlDelete, dados).then((response) => {
        ...
        xhr: () => {
          ...
          if (ajax.upload) {
            ...
            ajax.upload.addEventListener(
              "progress",
              (e) => {
                refVue.arquivo.pctProgresso = Math.round(
